...
namespace BlazorDemo.DataProviders {
    ...
    public class EntityDataProvider<T> : EntityDataProviderBase<T> where T : DbContext {
        ...
        protected async Task InsertAsync<TEntity>(string dbSetKey, IDictionary<string, object> newValues, Action<IQueryable<TEntity>, TEntity> updateKeyFunc, CancellationToken ct = default) where TEntity : class, new() {
            await InsertAsync<TEntity, TEntity>(dbSetKey, newValues, updateKeyFunc, updateKeyFunc, ct);
        }
        protected async Task InsertAsync<TDbSetEntity, TItemEntity>(string dbSetKey, IDictionary<string, object> newValues, Action<IQueryable<TDbSetEntity>, TDbSetEntity> updateDbKeyFunc, Action<IQueryable<TItemEntity>, TItemEntity> updateItemKeyFunc, CancellationToken ct = default) where TDbSetEntity : class, new() where TItemEntity : class, new() {
            if(UseDataProxy) {
                var dataWrapper = GetDataWrapper<TItemEntity>(dbSetKey);
                await dataWrapper.Add(newValues, UpdateItemProperties, updateItemKeyFunc);
            } else {
                using(var ctx = ContextFactory.CreateDbContext()) {
                    var dbSet = ctx.Set<TDbSetEntity>();
                    TDbSetEntity dataItem = new TDbSetEntity();
                    UpdateItemProperties(dataItem, newValues);
                    updateDbKeyFunc(dbSet, dataItem);
                    dbSet.Add(dataItem);
                    await ctx.SaveChangesAsync();
                }
            }
        }
        protected async Task InsertAsync<TEntity>(string dbSetKey, TEntity dataItem, Action<IQueryable<TEntity>, TEntity> updateKeyFunc, CancellationToken ct = default) where TEntity : class, new() {
            await InsertAsync<TEntity, TEntity>(dbSetKey, dataItem, updateKeyFunc, updateKeyFunc, ct);
        }
        protected async Task InsertAsync<TDbSetEntity, TItemEntity>(string dbSetKey, TItemEntity dataItem, Action<IQueryable<TDbSetEntity>, TDbSetEntity> updateDbKeyFunc, Action<IQueryable<TItemEntity>, TItemEntity> updateItemKeyFunc, CancellationToken ct = default) where TDbSetEntity : class, new() where TItemEntity : class, new() {
            if(UseDataProxy) {
                var dataWrapper = GetDataWrapper<TItemEntity>(dbSetKey);
                TItemEntity newDataItem = new TItemEntity();
                UpdateItemProperties(newDataItem, dataItem);
                await dataWrapper.Add(newDataItem, updateItemKeyFunc);
            } else {
                using(var ctx = ContextFactory.CreateDbContext()) {
                    var dbSet = ctx.Set<TDbSetEntity>();
                    TDbSetEntity newDataItem = new TDbSetEntity();
                    UpdateItemProperties(newDataItem, dataItem);
                    updateDbKeyFunc(dbSet, newDataItem);
