...
*   `Auto` (default) — The filter tree mode depends on the bound data source. When bound to a [GridDevExtremeDataSource\<T>](https://docs.devexpress.com/Blazor/DevExpress.Blazor.GridDevExtremeDataSource-1), the TreeList component switches to the `Nodes` mode to improve performance. In other data binding scenarios, `ParentBranch` mode is used.
